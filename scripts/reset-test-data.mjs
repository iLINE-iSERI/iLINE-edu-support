...
const [appIds, keyIds, settleIds, outputIds] = await Promise.all([
...
const line = (label, n) => console.log(`   ${label.padEnd(28, ' ')} ${n} 건`)

if (onlyUids) {
  console.log('🎯 대상을 좁혔습니다 — 아래 사람 것만 건드립니다')
  onlyEmails.forEach((e) => {
    const u = users.find((x) => x.email === e)
    console.log(`   ${e.padEnd(34, ' ')} ${u?.name || '(회원 등록 미완료)'}`)
  })
  console.log('')
}
...
if (accountsToDelete.length > 0) {
  ...
  accountsToDelete.forEach((a) => {
    ...
    console.log(`   ${(a.email || '(이메일 없음)').padEnd(34, ' ')} ${u?.name || '(회원 등록 미완료)'}`)
  })
}
...
if (!keepAccounts) {
  if (keepUids.size > 0) {
    ...
    ;[...keepUids].forEach((uid) => {
      ...
      console.log(`   ${(u?.email || a?.email || uid).padEnd(34, ' ')} ${u?.name || ''} (${why})`)
    })
  } else {
