# Firebase에서 직접 공고 등록하는 법 (예외 상황용)

> 사업 안에 여러 프로그램이 있고 프로그램마다 신청 방식이 다르므로,
> 프로그램을 먼저 등록해야 `/apply` 목록에 나타납니다.

✅ **담당자용 등록 화면이 생겼습니다 (09-06).** 사이트에서 **[신청 관리] →
[프로그램 관리]** 로 등록·수정하시면 됩니다. 사용법은 **`docs/1-운영/01-공고-등록-안내서.md`**.

아래 콘솔 방식은 **예외 상황용 참고 자료**로 남겨 둡니다. 콘솔 권한은 컬렉션
단위로 나눌 수 없어 신청자 개인정보까지 함께 열리므로, 평소에는 쓰지 마세요.

⚠️ **먼저 보안 규칙이 적용되어 있어야 합니다** (`docs/3-할일/01-남은-일.md` A항목).
...
자세한 배경은 `docs/4-기록/01-신청서-설계.md` 를 보세요.
