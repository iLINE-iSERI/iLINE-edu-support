  ...
  theme: {
    extend: {
      ...
      boxShadow: {
        ...
        /**
         * 떠 있어야 하는 것 — 목록의 항목·홈 접수중 카드 (D-103).
         *
         * 🔴 **`card` 의 값을 올려서 해결할 수 없었다.** 그 토큰은 40곳 넘게 쓰이고
         *    상당수가 카드가 아니라 **신청서 폼 섹션**이다(`ApplicationForm` 6 ·
         *    전용 양식 8 · `ProgramFields` 4). 신청서 한 화면에 흰 섹션이 열 개
         *    안팎 쌓이므로 거기에 강한 그림자가 붙으면 **화면이 무거워진다.**
         *    값을 올리는 대신 **용도를 나눴다.**
         *
         * ⚠️ **호버는 반드시 기본보다 강해야 한다.** 기존 `card-hover`(6%)는 새
         *    `raised`(12%)보다 **약하다** — 그대로 물려 쓰면 마우스를 올렸을 때
         *    그림자가 **오히려 옅어진다.** 그래서 호버 값을 함께 만들었다.
         */
        raised: '0 1px 2px rgb(15 23 42 / 0.07), 0 8px 20px -4px rgb(15 23 42 / 0.12)',
        'raised-hover':
          '0 2px 4px rgb(15 23 42 / 0.08), 0 14px 28px -6px rgb(15 23 42 / 0.18)',
      },
      fontFamily: {
        sans: ['var(--font-sans)'],
      },
