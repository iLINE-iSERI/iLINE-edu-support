 ...
 *
 * ── 팀 대표자만 쓰는 것 ──────────────────────────────────
 * 트랙·과제·학교급·교과·제출 서류(수업설계안·선택한 이유·**지원 동기**)는
 * **대표자만** 작성한다(AI-EDU 의 팀장 방식과 같다). 두 사람이 각각 다른
 * 과제를 적으면 어느 것이 팀의 지원 과제인지 알 수 없다.
 *
 * ⚠️ **지원 동기도 대표자만이다 — 09-24 에 교수님이 확인해 주신 것.**
 *    한 번 뒤집으려다 되돌렸다. 팀원 화면에 지원 동기 칸이 없는 것이
 *    **빠뜨린 것이 아니라 의도**다. 다음에 "팀원 화면에 지원 동기가 없네"
 *    하고 고치기 전에 **운영에 먼저 물을 것.**
 *
 *    ▸ 09-24 에 실제로 생긴 일: 교수님이 팀원 화면에 칸이 없는 것을 보고
 *      **공고의 자율 칸(D-99)으로 「지원 동기」를 또 만드셨고**, 그 바람에
 *      대표자 화면에 지원 동기가 **두 개**가 됐다. 해결은 코드가 아니라
 *      **그 자율 칸을 지우는 것**이었다.
 *    ▸ 구조적으로 남는 것: **자율 칸은 「더하기」만 되고 「양식 안의 칸 끄기」는
 *      안 된다.** 그래서 양식이 **조건에 따라 감추는 항목**이 있으면 담당자는
 *      우회할 수밖에 없고 반드시 중복이 생긴다. 담당자 화면의 경고에 그 사실을
 *      적어 두었다(`app/staff/programs` 의 「신청 양식」 칸 아래).
 *
 * ── 참가 유의사항은 여기 없다 (D-98) ─────────────────────
