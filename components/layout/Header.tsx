 ...
 * 대메뉴 (D-13).
 *
 * ⚠️ **갤러리를 뺐다 (09-08).** 화면은 `app/gallery/page.tsx` 에 그대로
 *    있지만 **메뉴에서는 내렸다.** 내용이 하나도 없는 상태라, 눌렀을 때
 *    빈 화면이 뜨면 이용자는 **고장으로 받아들인다.**
 *
 *    교수님이 갤러리보다 **시설 예약을 먼저** 하기로 하셨으므로(D-47)
 *    접수 개시까지 채워질 가능성이 없다. 산출물이 쌓이면 이 줄만 되살리면
 *    된다 — **화면은 지우지 않았다.**
 */
const NAV = [
