 ...
 * 첨부란이 나타난다. 설계 배경은 docs/4-기록/01-신청서-설계.md 참고.
 */
...
export default function ApplicationForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // 전용 항목은 화면에서 위쪽에 있으므로 검증도 먼저 한다.
    // 아래 칸을 먼저 지적하면 위에 있는 문제를 나중에 발견하게 된다.
    if (form) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* ── 이 프로그램 전용 항목 (D-50) ─────────────────────
          프로그램에 양식이 걸려 있을 때만 나타난다. 이 화면은 그 안에
          무엇이 있는지 모른다 — 양식 파일이 화면과 검증을 모두 갖는다. */}
      {form && (
        <form.Component
          value={extra}
